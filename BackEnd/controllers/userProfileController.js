...
exports.updateUserProfile = async (req, res) => {
    try {
        ...
        await UserProfile.updateOne({ userid: req.user._id }, { $set: filteredBody })
...
exports.addFollowers = async (req, res) => {
    try {
        ...
        await UserProfile.updateOne({ _id: sender._id }, { $set: { following: arr } })
        await UserProfile.updateOne({ _id: receiver._id }, { $set: { followers: arr2 } })
        res.status(200).json({
            status: "success"
        })
...
exports.editPost = async (req, res) => {
    try {
        ...
        const result = await UserPost.updateOne({ _id: req.body.id }, { ...filteredBody })
        res.status(200).json({
            status: 'success',
            message: {
                data: result
...
exports.addComment = async (req, res) => {
    ...
    try {
        ...
        const result = await UserPost.updateOne({ _id: postid }, { $set: { comment: newComments } })
        res.status(200).json({
            status: 'success',
            data: result
        })
