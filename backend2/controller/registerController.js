...
const verifyMail = (name, email, id) => {
  ...
  const mailOptions = {
    ...
    html:
      "<p>Hi " +
      name +
      ".Verify Your Email</p><br><a href='http://localhost:8000/registeration/verify?id=" +
      id +
      "'>Verify</a>!!!!!",
  };
  ...
  if (user.id) {
    ...
    try {
      ...
      res.send({
        id: user._id,
        token,
        message: "A Link to Your Email has been sent. Verify Your Email",
      });
    } catch (error) {
      console.log(error.message);
    }
...
const verifyUser = async (req, res) => {
  try {
    ...
    res.send(
      "Your Email Has been verified.You will now be redirected to set your password"
    );
  } catch (error) {
    console.log(error);
  }
