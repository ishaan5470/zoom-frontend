    ...
    if(user){
      ...
      bcrypt.hash(generatedOtp, 10, async function (err, hash) {
          ...
          if (err) {
            ...
            const hashString = hash.toString();
            const userData = await User.updateOne({email:email},{phoneOtp:hashString})
            let token=jwt.sign({ phoneNumber:req.body.phoneNumber }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '3m' })
            res.cookie(`token`,`${token}`,{httpOnly:true,secure:true})
            res.status(200).json({
              status: "sucess",
    ...
    if (userData) {
      bcrypt.compare(req.body.otp, userData.phoneOtp, async function (err, result) {
        ...
        if (err) {
          ...
          if (result) {
            const updatedInfo = await User.updateOne({ phoneNumber:userData.phoneNumber }, { $set: { is_verified_phone: true,phoneOtp:"" } })
...
const forgotPasswordSetPassword = async(req,res)=>{
    ...
    if(req.body.email){
      ...
      if(user){
          ...
          password = hashedPass;
          const userData = await User.updateOne({email:req.body.email},{password:password})
      ...
      if(user){
          ...
          const hashedPass = await bcrypt.hash(password, salt);
          password = hashedPass;
          const userData = await User.updateOne({phoneNumber:req.body.phoneNumber},{password:password})
            console.log(userData,"Password has been updated");
            let accessToken = jwt.sign({ name: user.name }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '2m' })
            let refreshToken = jwt.sign({ name: user.name }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: '1h' })
            res.cookie({ refreshToken });
